...
if __name__ == "__main__":
    ...
    """d = {
        ...
        'val_length': 10,
        'batch_size': 2,
        'classifications_file': "classifications_%s_%s.pkl" % ("test", "0"),
        'lr': 0.01
    }
    ...
    for inp_ in run.train_loader:
        inp = inp_
        break
    ct, pet, merged, label, _ = inp
    inp = torch.Tensor(merged.float())
    inp = inp.to(device)
    run.writer.add_graph(run.model, input_to_model=inp)
    print(run.writer.log_dir)
    run.writer.close()

    cross_validation_fold = 12 # !!!"""
    with open("./runs_grid_search/classifications_MyModel_1.pkl", "rb") as f:
        cls = pickle.load(f)
    for d in cls:
        print(d)
    print(cls['model_version'])

    model_path = "./runs/Apr19_22-42-36_leon-desktop/best_val.pth"
